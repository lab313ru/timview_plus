...
type
 ...
 (* {$IFNDEF UseDelphi}
  ...
  public
   ...
   // procedure AssignTo(Dest: TPersistent);{$IFDEF UseDelphi}override;{$ENDIF}
    ...
    procedure LoadFromStream(Stream: PStream);
      {$IFDEF UseDelphi}override;{$ENDIF}
    procedure SaveToStream(Stream: PStream); {$IFDEF UseDelphi}override;{$ENDIF}
  ...
  public
    ...
    function SaveData(Stream: PStream): Boolean;
...
function TChunk.SaveData(Stream: PStream): Boolean;
var
...
function TChunktIME.LoadFromStream(Stream: PStream;
...
function TChunktIME.SaveToStream(Stream: PStream): Boolean;
begin
  {Update data}
...
{Loading the chunk from a stream}
function TChunkzTXt.LoadFromStream(Stream: PStream;
  const ChunkName: TChunkName; Size: Integer): Boolean;
var
  ErrorOutput: String;
...
{Saving the chunk to a stream}
function TChunkztXt.SaveToStream(Stream: PStream): Boolean;
var
  Output: Pointer;
  OutputSize: Integer;
  ErrorOutput: String;
...
{Loading the chunk from a stream}
function TChunktEXt.LoadFromStream(Stream: PStream;
  const ChunkName: TChunkName; Size: Integer): Boolean;
begin
  {Load data from stream and validate}
  Result := inherited LoadFromStream(Stream, ChunkName, Size);
...
{Saving the chunk to a stream}
function TChunktEXt.SaveToStream(Stream: PStream): Boolean;
begin
  {Size is length from keyword, plus a null character to divide}
...
{Chunk being loaded from a stream}
function TChunkIHDR.LoadFromStream(Stream: PStream; const ChunkName: TChunkName;
  Size: Integer): Boolean;
begin
  {Let TChunk load it}
  Result := inherited LoadFromStream(Stream, ChunkName, Size);
...
function TChunkIHDR.SaveToStream(Stream: PStream): Boolean;
begin
...
{Saving the chunk to a stream}
function TChunktRNS.SaveToStream(Stream: PStream): Boolean;
begin
...
{Loads the chunk from a stream}
function TChunktRNS.LoadFromStream(Stream: PStream; const ChunkName: TChunkName;
  Size: Integer): Boolean;
var
  i, Differ255: Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Integer(fStream.Position) = EndPos) and (avail_out > 0) and
        (avail_in = 0) then
      begin
        ...
        {$ENDIF}
        EndPos := Integer(fStream.Position) + ByteSwap(EndPos);
      end;
      ...
      begin
        ...
        if Integer(fStream.Position) + ZLIBAllocate > EndPos then
          avail_in := fStream.Read(Data^, EndPos - Integer(fStream.Position))
         else
...
function TChunkIDAT.LoadFromStream(Stream: PStream; const ChunkName: TChunkName;
  Size: Integer): Boolean;
var
...
begin
  ...
  EndPos := Integer(Stream.Position) + Size;
...
function TChunkIDAT.SaveToStream(Stream: PStream): Boolean;
var
  ZLIBStream : TZStreamRec2;
begin
  {Get pointer to the header chunk}
...
procedure WriteIDAT(Stream: PStream; Data: Pointer; const Length: Cardinal);
var
...
function TChunkPLTE.LoadFromStream(Stream: PStream;
  const ChunkName: TChunkName; Size: Integer): Boolean;
type
  pPalEntry = ^PalEntry;
  PalEntry = record
...
function TChunkPLTE.SaveToStream(Stream: PStream): Boolean;
var
  J: Integer;
  DataPtr: pByte;
  BitmapInfo: TMAXBITMAPINFO;
...
{Loading the chunk from a stream}
function TChunkgAMA.LoadFromStream(Stream: PStream;
  const ChunkName: TChunkName; Size: Integer): Boolean;
var
  i: Integer;
  Value: Cardinal;
...
procedure TPngObject.LoadFromStream(Stream: PStream);
var
...
begin
  ...
  repeat
    ...
    begin
      dec(ChunkCount);
      Stream.Seek(ChunkLength + 4, spCurrent);
      Continue;
    end;
...
begin
  ...
  Header.Height := abs(Info.Height);
  {Set bit depth}
  if Info.BitsPerPixel >= 16 then
    Header.BitDepth := 8 else Header.BitDepth := Info.BitsPerPixel;
  {Set color type}
  if Info.BitsPerPixel >= 16 then
...
{Loads the chunk from a stream}
function TChunkpHYs.LoadFromStream(Stream: PStream; const ChunkName: TChunkName;
  Size: Integer): Boolean;
begin
  {Let ancestor load the data}
  Result := inherited LoadFromStream(Stream, ChunkName, Size);
...
{Saves the chunk to a stream}
function TChunkpHYs.SaveToStream(Stream: PStream): Boolean;
begin
  {Update data}
